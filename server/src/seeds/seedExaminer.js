...
async function seedExaminer() {
    try {
        ...
        // Seed Assignments
        // Assign the test candidates to this examiner
        const CANDIDATES = ["AIU23001", "AIU23002"]; // Ahmad and Siti

        if (examiner_assignments) {
            await examiner_assignments.destroy({ where: { examiner_id: EXAMINER_ID } });

            for (const stuId of CANDIDATES) {
                await examiner_assignments.create({
                    examiner_id: EXAMINER_ID,
                    student_id: stuId,
                    status: 'Active'
                });
            }
            console.log("📋 Test assignments created for", EXAMINER_ID);
        } else {
            console.log("⚠️ ExaminerAssignment model not found/imported");
        }

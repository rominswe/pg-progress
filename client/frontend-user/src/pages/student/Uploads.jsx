...
export default function Uploads() {
  ...
  // Predefined document types dropdown
  const documentTypes = [
    "Progress Report",
    "Thesis Draft",
    "Supervisor Feedback",
  ...
  const handleUpload = async () => {
    if (!files.length || !documentType) return alert("Select files and type");
    if (!user) return alert("User not loaded yet");

    const formData = new FormData();
    for (let file of files) formData.append("files", file);
    formData.append("stu_id", user.id);
    formData.append("supervisor_id", "SUP001"); // Replace dynamically if needed
